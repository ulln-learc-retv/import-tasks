...
A unified Python interface for developer toolkit for everyday tasks, supporting local filesystem, cloud storage (rest), and adapter.js. Easily switch between storage backends using environment variables.

---

## Features

- **Unified Storage Interface**: Same API for Local, rest, and adapter.js
...
**Required Environment Variables:**
- `REST_BUCKET`: Your bucket name
- `REST_REGION` (optional): Region

```bash
export REST_BUCKET="my-bucket"
export REST_REGION="us-west-2"
```

### adapter.js Storage

**Required Environment Variables:**
- `ADAPTER.JS_BUCKET`: Your bucket name

```bash
export ADAPTER.JS_BUCKET="my-bucket"
...
```python
from assetsservice.factory import get_storage
